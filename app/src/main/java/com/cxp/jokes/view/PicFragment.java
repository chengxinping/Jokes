...
import com.cxp.jokes.presenter.PicJokesPresenter;
...
public class PicFragment extends BaseFragment implements BaseView {
    private int maxResult;
    private String timestamp;
    private XRecyclerView mRecyclerView;
    private ProgressBar mProgressBar;
    private List<PicJokesModel.ShowapiResBodyBean.ContentlistBean> mList;
    ...
    private void initPresenter() {
        maxResult = 10;
        PicJokesPresenter presenter = new PicJokesPresenter(this);
        presenter.getData(timestamp, maxResult);
    }

    @Override
    public void success(Object object) {
        mList = ((PicJokesModel) object).getShowapi_res_body().getContentlist();
        mRecyclerView.setLayoutManager(new LinearLayoutManager(mActivity));
        mRecyclerView.setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader);
        mRecyclerView.setLoadingMoreProgressStyle(ProgressStyle.SquareSpin);
        mRecyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), DividerItemDecoration.VERTICAL));
        mRecyclerView.setAdapter(new CommonAdapter<PicJokesModel.ShowapiResBodyBean.ContentlistBean>(mActivity, R.layout.item_pic_jokes, mList) {

            @Override
            protected void convert(ViewHolder holder, final PicJokesModel.ShowapiResBodyBean.ContentlistBean contentlistBean, int position) {
        ...
        mRecyclerView.setLoadingListener(new XRecyclerView.LoadingListener() {
            ...
            public void onLoadMore() {
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        PicJokesPresenter presenter = new PicJokesPresenter(PicFragment.this);
